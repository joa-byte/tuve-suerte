...
export function withAverages(dinner) {
  const guests = [...new Set(
    [...dinner.dishes, ...dinner.wines]
      .flatMap(item => item.reviews)
      .map(review => String(review.author || '').trim())
      .filter(Boolean)
  )];
  return {
    ...dinner,
...
export function createD1Repository(db) {
  ...
  function hydrateDinner(row, dishRows, wineRows, reviewRows) {
  ...
  return {
    async listDinners() {
      const [dinnerResult, dishResult, wineResult, reviewResult] = await Promise.all([
      ...
      const related = [results(dishResult), results(wineResult), results(reviewResult)];
    ...
    async getDinner(id) {
      ...
      if (!row) return null;
      const [dishResult, wineResult, reviewResult] = await Promise.all([
      ...
      return hydrateDinner(row, results(dishResult), results(wineResult), results(reviewResult));
    },

    async createDinner(input) {
      const id = createId(input.title);
      await db.prepare('INSERT INTO dinners (id, title, date) VALUES (?, ?, ?)')
        .bind(id, input.title, input.date)
        .run();
...
export function handleDinners(context) {
  return safely(async () => {
    ...
    const dinner = await repository.createDinner({ title, date: input.date });
